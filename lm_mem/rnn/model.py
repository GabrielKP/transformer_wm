...
class RNNModel(nn.Module):
    ...
    def __init__(
        ...
        if rnn_type in ["LSTM", "GRU"]:
            ...
            self.rnn = LSTMWithStates(
                ninp, nhid, nlayers, dropout=dropout, store_states=store_states
            )
        else:
            try:
...
class LSTMStates(object):
    ...
    def __repr__(self):
        ...
        return "LSTMOutput(L{}, ({}, {}))".format(self.layer + 1, len(self.h), self.h[0].shape)
...
class LSTMWithStates(nn.LSTM):
    ...
    def __init__(self, ninp, nhid, nlayers, dropout=0.5, store_states=True):
